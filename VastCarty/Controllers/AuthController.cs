...
namespace VastCarty.Controllers
{
    public class AuthController : Controller
    {
        ...
        // store pending registration OTPs in-memory (short-term)
        private static readonly Dictionary<int, string> _pendingRegistrations = new();
        private readonly ISmsServices _smsServices;
        private string returnUrl;

        public AuthController(ILogger<AuthController>logger, IEmailServices emailServices, IUserServices userServices,ISmsServices smsServices)
        {
        ...
        public async Task<IActionResult> Register([FromForm] RegisterViewModel model)
        {
            ...
            try
            {
                ...
                var existingUser = await _userServices.GetUserByEmailAsync(model.Email);
                ...
                var userId = await _userServices.CreateUsersAsync(user);
                ...
                // Send OTP via email
                await _emailServices.SendRegistrationOTPAsync(user.Email, otp, user.UserName);

                // Send OTP via SMS if phone provided
                if (!string.IsNullOrEmpty(model.PhoneNumber))
                {
                    await _smsServices.SendRegistrationOTPAsync(model.PhoneNumber, otp);
                }
        ...
        public async Task<IActionResult> VerifyRegistration(int? userId)
        {
            ...
            var user = await _userServices.GetUsersByIdAsync(finalUserId);
        ...
        public async Task<IActionResult> Verify(int? userId)
        {
            ...
            var user = await _userServices.GetUsersByIdAsync(userId.Value);
        ...
        public async Task<IActionResult> VerifyRegistrationOTP([FromBody] VerifyOTPRequest request)
        {
            try
            {
                ...
                var user = await _userServices.GetUsersByIdAsync(request.UserId);
                if (user != null)
                {
        ...
        public async Task<IActionResult> ResendRegistrationOTP([FromBody] ResendOTPRequest request)
        {
            try
            {
                ...
                // Resend OTP
                await _emailServices.SendRegistrationOTPAsync(user.Email, newOtp, user.FullName);

                if (!string.IsNullOrEmpty(user.PhoneNumber))
                {
                    await _smsServices.SendRegistrationOTPAsync(user.PhoneNumber, newOtp);
                }
        ...
        // Minimal sign-in helper to satisfy callers; implement real authentication flow as needed.
        private Task SignInUserAsync(VastCartyDBL.Models.Users user, bool rememberMe)
        {
            // TODO: implement proper authentication (ClaimsPrincipal, cookies, etc.)
